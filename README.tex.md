...
$$
where $g = \mathtt{gsw_grav(latitude, p)}$, $\rho_\text{p\_mid} = \mathtt{gsw\_rho(SA, CT, p\_mid)}$, $\rho^\Theta_\text{p\_ref} = \mathtt{gsw\_rho(SA, CT, p\_ref)}$, $\text{p\_mid}$ is the average pressure between two depths and the different references pressures $\text{p\_ref} = (0, 1000, 2000, 3000, 4000)$ dbar.
